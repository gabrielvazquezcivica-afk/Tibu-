...
export function toSticker(buffer, isVideo = false) {
    return new Promise((resolve, reject) => {
        ...
        const cmd = isVideo
 ? `ffmpeg -i "${input}" -vcodec libwebp_anim -vf "fps=10,scale=320:320:force_original_aspect_ratio=decrease:flags=lanczos,pad=320:320:(ow-iw)/2:(oh-ih)/2:color=0x00000000" -loop 0 -an -vsync 0 -q:v 75 -y "${output}"`
